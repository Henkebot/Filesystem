...
int main(int argc, char ** argv)
{
	...
	pthread_t uiID;							//Skapar tråd för user input
	pthread_t threadIDs[nThreads];			//skapar analystrådar.
	...
	pthread_create(&uiID, NULL, userInteraction, NULL);		//Säger åt ui tråden att arbeta
	...
	for(int i=0 ; i<nThreads ; i++)
	{
		...
		pthread_create(&threadIDs[i], NULL, frequencyAnalysis, (void*)params);
	}
	for (int i = 0; i < nThreads; i++)
	{
